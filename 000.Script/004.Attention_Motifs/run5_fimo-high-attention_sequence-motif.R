...
####### 文件合并及读入，并添加标注符
A <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/Fimo_HERV_Coding/fimo.tsv", header=TRUE)
B <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/Fimo_HERV_Non-Coding/fimo.tsv", header=TRUE)
C <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/Fimo_Non-HERV_Coding/fimo.tsv", header=TRUE)
D <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/Fimo_Non-HERV_Non-Coding/fimo.tsv", header=TRUE)
### 第一分组
...
pdf("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Overlap.pdf")
...
# 1. 创建画布
#ggsave("BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Overlap_NonLabels.png",grid.draw(venn.plot1),dpi = 2000)
dev.off()

##
png(filename = "BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Overlap_NonLabels.png",bg = "white")
par(family="Times New Roman")
grid.draw(venn.plot1)
dev.off()
...
write.table(motif_counts_norm_infor, file = "/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment.txt", sep = "\t", row.names = FALSE,quote=FALSE)
...
# 对每个Category选取最多20行
out1<-out1 %>% group_by(Category) %>% slice_head(n = 10)

pdf("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment.pdf")
par(family="Times New Roman")
...
p1<-ggplot(out1, aes(HERV_Rate,Class)) +
  ...
  theme(legend.direction = "vertical", legend.position = "right",axis.text.y=element_text(vjust=1,size=7,lineheight=10),axis.text.x=element_text(angle=45),legend.text = element_text(size=5),legend.title = element_text(size=7),text = element_text(family = "Times New Roman"))+#legend水平置于顶部
  scale_y_discrete(position = "right")#y轴文字放右侧

p1
p1 %>% insert_left(Category, width =0.05)

CairoPNG("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment.png",width=6000, height=8000,dpi=2000, family="Times New Roman")
p1 %>% insert_left(Category, width =0.05)
...
######## 3. 富集Motifs的交集
A <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment.txt", header=TRUE, sep="\t")
B <- read.table("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment_Raw.txt", header=TRUE,  sep="\t")

A$Group <- "High_Attention"
B$Group <- "ALL"
AA_data<-rbind(subset(A,p_hyper_adjust<=0.05),subset(B,p_hyper_adjust<=0.05))

A_F <- unique(subset(A,p_hyper_adjust<=0.05)$motif_alt_id)
B_F <- unique(subset(B,p_hyper_adjust<=0.05)$motif_alt_id)
gene_lists <- list(A=A_F, B=B_F)

### 韦恩图
venn.plot <- venn.diagram(
  x = gene_lists,
  category.names = c("High_Attention","ALL"),
  filename = NULL,
  col = "transparent",
  fill = c("red","green"),
  category.cex = 0.4,
  category.dist = 2,  # 进一步增加标签的位置距离
  width = 50,  # 增大图像宽度
  height = 30  # 增大图像高度
)
venn.plot1 <- venn.diagram(
...
pdf("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment-Overlap.pdf")
grid.draw(venn.plot)
grid.newpage()
grid.draw(venn.plot1)
dev.off()
...
### 柱状图
pdf("/home/u20111010010/Project/DNA-Pretraining/Level1/003.Sequence_Visualization/Dataset_HERV/BERT_HERV_Multi_RUN0-Higher_Attention-Motif_visualization-Enrichment-Class_detail.pdf")

data_df <- as.data.frame(table(AA_data$Group, AA_data$Category))
colnames(data_df) <- c("Class","Type","Number")
ggplot(data_df, aes(Type,Number, fill = Class)) +ylim(0,56)+geom_bar(stat = 'identity', lwd = 1.5, position = position_dodge2(padding = 0,preserve = "single"))+labs(x = "HERV  Category", y = "The Number of Enriched Motifs", title=" ")+theme_classic()+theme(axis.text.x = element_text(angle=45, vjust = 0.5,face = "bold",size=7),axis.title =  element_text(size=15,face = "bold"),legend.title=element_text(face = "bold"),panel.grid=element_blank(),legend.position = c(0.75,0.8))
